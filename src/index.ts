...
async function gatherSearchContext(question: string): Promise<string> {
  try {
    ...
    // Extract text snippets from search result elements
    ...
    console.warn(`[Search Warning] Could not fetch live news for "${question}":`, err);
  }
...
/**
 * Pulls actual collateral token balance (TST) for the active wallet.
 */
async function getBankroll(): Promise<number> {
  try {
    ...
    const tokenBalance = Number(rawBalance) / 1e6; // 6 decimals for TST / USDC
    console.log(`Current wallet bankroll: ${tokenBalance.toFixed(2)} TST`);
    return tokenBalance > 0 ? tokenBalance : 1000;
  } catch (err) {
    console.warn("[Bankroll Warning] Could not query token balance on-chain, falling back to 1000:", err);
    return 1000;
  }
}
...
async function runOnce(): Promise<void> {
  ...
  console.log("\n1. Redeeming settled positions...");
  const { redeemed } = await redeemSettledPositions();
  console.log(`Redeemed ${redeemed} settled position(s).\n`);
  ...
  console.log("3. Fetching bankroll balance...");
